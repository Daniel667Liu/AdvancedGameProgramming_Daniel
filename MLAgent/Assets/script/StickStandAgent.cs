...
public class StickStandAgent : Agent
{
    ...
    {
        float InitialRotation = UnityEngine.Random.Range(-3f, 3f);
        var scale = 1;
        m_BallRb.mass = scale;
        ball.transform.position = new Vector3(UnityEngine.Random.Range(-0.2f, 0.2f), 2f, UnityEngine.Random.Range(-0.2f, 0.2f))
            + gameObject.transform.position;
        m_BallRb.velocity = new Vector3(0, 0, 0);
        //m_BallRb.inertiaTensorRotation = new Quaternion(0.1f, 0.1f, 0.1f, 0.1f);
        //m_BallRb.ResetCenterOfMass();//reset the center of mass, or the stick would rotate itself
        ball.transform.localScale = new Vector3(0.5f*scale, scale,0.5f* scale);
        //ball.transform.rotation = Quaternion.Euler(InitialRotation, InitialRotation, InitialRotation);
        ball.transform.rotation = new Quaternion(0,0,0,0);
        

        
        //Debug.Log(ball.transform.rotation);
    }
    ...
    public override void OnActionReceived(ActionBuffers actions)//apply the actions from neural network
    {
        ...
        if (ball.transform.position.y - gameObject.transform.position.y < -3f//the ball fall down
            ...
            || (TopPosition.transform.position.y - MiddlePosition.transform.position.y) < 0.1f)
            //the stick fall down
        {
            SetReward(-1f);
            EndEpisode();
